...
<%
    ...
    if(user_info!=null && user_info.getUser_auth_key().getUser_type().equals(ADMIN)){
        if(professor!=null){
            webServicesStringUrl = "http://localhost:8080/FinalDPWebApp/webresources/Professors/invalidateProfessor?professor_id="+professor;            
        }
        responseString = FunctionsController.getResponse(webServicesStringUrl,"PUT");
        if(responseString!=null && responseString.contains("Error")){            
            //Found an Error.
            session.setAttribute("error", "invalidate_fail");            
        }
        response.sendRedirect("index.jsp");
    }else{
        if(cookies!=null)
